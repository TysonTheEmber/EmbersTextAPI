...
public final class SDFProviderRegistry {
    ...
    private static final Map<Identifier, List<GlyphProvider>> PROVIDERS = new ConcurrentHashMap<>();
    ...
    public static void register(Identifier fontName, GlyphProvider provider) {
        PROVIDERS.computeIfAbsent(fontName, k -> new ArrayList<>()).add(provider);
        LOGGER.info("Registered SDF provider for font '{}'", fontName);
    }

    @Nullable
    public static List<GlyphProvider> getProvidersForFont(Identifier fontName) {
