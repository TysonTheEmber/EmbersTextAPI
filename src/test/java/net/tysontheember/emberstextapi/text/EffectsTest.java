...
class EffectsTest {
    ...
    void gradientInterpolatesBetweenColors() {
        ...
        Attribute attr = new Attribute(new ResourceLocation("embers", "grad"), Params.of(Map.of(
                "from", "#000000",
                "to", "#FFFFFF"
        )));
        Span span = new Span(0, text.length(), List.of(attr));
        DrawStyle style = DrawStyle.builder().animationStartTime(0f).build();
        TextEffect effect = Effects.compile(attr, new TextEffect.CompileContext(text, span, style, null));
    ...
    void typewriterHidesCharactersUntilSpeedAllows() {
        ...
        Attribute attr = new Attribute(new ResourceLocation("embers", "typewriter"), Params.of(Map.of(
                "speed", 2
        )));
        Span span = new Span(0, text.length(), List.of(attr));
        DrawStyle style = DrawStyle.builder().animationStartTime(0f).build();
    ...
    void shakeProducesDeterministicOffsets() {
        ...
        Attribute attr = new Attribute(new ResourceLocation("embers", "shake"), Params.of(Map.of(
                "a", 2f,
                "f", 1f
        )));
        Span span = new Span(0, text.length(), List.of(attr));
        DrawStyle style = DrawStyle.builder().animationStartTime(0f).seed(1234L).build();
        TextEffect effect = Effects.compile(attr, new TextEffect.CompileContext(text, span, style, null));
