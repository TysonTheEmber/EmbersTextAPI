...
void main() {
    ...
    // Sharpen the AA edge by squaring alpha. This dramatically reduces the
    // contribution of low-alpha fringe fragments (0.1 → 0.01, 0.05 → 0.0025)
    // while preserving the smooth transition at the glyph edge (0.5 → 0.25,
    // 0.9 → 0.81). Prevents visible halos on MC versions where the compositing
    // pipeline doesn't fully hide semi-transparent edge fragments.
    alpha *= alpha;
